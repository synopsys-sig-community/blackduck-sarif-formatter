                ...
                for licenseViolation in component["policyViolationLicenses"]:
                    for violatingPolicies in licenseViolation["violatingPolicies"]:
                        ...
                        if not ruleId in ruleIds:
                            ...
                            rule = {"id":ruleId, "helpUri": licenseViolation['_meta']['href'], "shortDescription":{"text":f'{violatingPolicies["policyName"]}: {component["componentName"]}'}[:1024], 
                                "fullDescription":{"text":f'{violatingPolicies["description"][:1024] if "description" in violatingPolicies else "-"}', "markdown": f'{violatingPolicies["description"] if "description" in violatingPolicies else "-"}'},
